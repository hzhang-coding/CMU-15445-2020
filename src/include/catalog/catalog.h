...
class Catalog {
 public:
  ...
  TableMetadata *CreateTable(Transaction *txn, const std::string &table_name, const Schema &schema) {
    ...
    tables_[table_oid] = std::make_unique<TableMetadata>(schema, table_name, std::move(table_heap), table_oid);
  ...
  TableMetadata *GetTable(const std::string &table_name) {
    if (names_.count(table_name) == 0) {
      throw std::out_of_range("The table do not exist!");
    }
  ...
  IndexInfo *CreateIndex(Transaction *txn, const std::string &index_name, const std::string &table_name,
                         ...
                         size_t keysize) {
    BUSTUB_ASSERT(index_names_.count(table_name) != 0, "The table do not exist!");
    ...
    for (auto it = table->Begin(txn); it != table->End(); ++it) {
      indexes_[index_oid].get()->index_->InsertEntry(it->KeyFromTuple(schema, key_schema, key_attrs), it->GetRid(),
                                                     txn);
    }
  ...
  IndexInfo *GetIndex(const std::string &index_name, const std::string &table_name) {
    if (index_names_.count(table_name) == 0 || index_names_[table_name].count(index_name) == 0) {
      throw std::out_of_range("The index do not exist");
    }

    return indexes_[index_names_[table_name][index_name]].get();
  }
  ...
  std::vector<IndexInfo *> GetTableIndexes(const std::string &table_name) {
    ...
    for (auto &pair : index_names_[table_name]) {
      result.emplace_back(GetIndex(pair.second));
    }
